      ...
      <div className="mb-8 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-3xl font-extrabold mb-2 text-white">
            <span className="bg-gradient-to-r from-yellow-300 via-yellow-400 to-yellow-500 bg-clip-text text-transparent drop-shadow-lg">Support Tickets</span>
          </h1>
          <p className="mt-1 text-sm text-yellow-400">Create and track your support tickets</p>
        </div>
        <button
      ...
      <div className="space-y-6">
        {tickets.length === 0 ? (
          <div className="bg-gradient-to-br from-[#08152F]/95 via-[#0C1A6B]/90 to-[#05627C]/85 backdrop-blur-xl rounded-2xl shadow-2xl border border-yellow-500/30 p-12 text-center">
            <p className="text-yellow-400 text-lg">No tickets found. Create your first ticket to get started.</p>
          </div>
        ) : (
          tickets.map((ticket) => (
            <div key={ticket.id} className="bg-gradient-to-br from-[#08152F]/95 via-[#0C1A6B]/90 to-[#05627C]/85 backdrop-blur-xl rounded-2xl shadow-2xl border border-yellow-500/30 p-6 hover:border-yellow-500/60 hover:shadow-yellow-500/20 transition-all duration-300">
              <div className="flex justify-between items-start mb-4">
                ...
                <span className={`px-4 py-1.5 text-xs font-bold rounded-full shadow-lg ${
                  ticket.status === 'Open'
                    ...
                    ? 'bg-[#08152F]/80 text-yellow-200 border border-yellow-500/30'
                    : ticket.status === 'Closed'
                    ? 'bg-[#08152F]/80 text-yellow-300 border border-yellow-500/30'
                    : 'bg-[#08152F]/80 text-yellow-300 border border-yellow-500/30'
                }`}>
                  {ticket.status}
                </span>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div className="p-4 bg-[#08152F]/80 rounded-xl border border-yellow-400/30">
                  <p className="text-sm text-yellow-400 font-semibold mb-2">Department</p>
                  <p className="text-sm font-extrabold text-white">{ticket.department}</p>
                </div>
                {ticket.service && (
                  <div className="p-4 bg-[#08152F]/80 rounded-xl border border-yellow-400/30">
                    <p className="text-sm text-yellow-400 font-semibold mb-2">Service</p>
              ...
              {ticket.description && (
                <div className="mb-4 p-4 bg-[#08152F]/80 rounded-xl border border-yellow-400/30">
                  <p className="text-sm text-yellow-400 font-semibold mb-3">Description</p>
                  <p className="text-sm text-yellow-400 whitespace-pre-wrap">{ticket.description}</p>
                </div>
              )}
              ...
              {ticket.reply && (
                <div className="mt-4 p-5 bg-gradient-to-r from-yellow-500/20 via-yellow-600/15 to-yellow-500/20 border-2 border-yellow-500/40 rounded-xl">
                  ...
                  <p className="text-sm text-yellow-400 whitespace-pre-wrap">{ticket.reply}</p>
            ...
            <div className="mt-3">
              <div className="flex justify-between items-center mb-6">
                <h3 className="text-xl font-extrabold text-white flex items-center gap-2">
                  <span className="bg-gradient-to-r from-yellow-300 via-yellow-400 to-yellow-500 bg-clip-text text-transparent">Create Support Ticket</span>
                </h3>
                <button
                  onClick={() => {
                    setShowCreateModal(false);
              ...
              <form onSubmit={handleCreateTicket} className="space-y-5">
                <div>
                  ...
                  <select
                    ...
                    className="w-full px-4 py-3 border border-yellow-500/40 rounded-xl bg-[#08152F]/80 text-white focus:outline-none focus:ring-2 focus:ring-yellow-500/50 focus:border-yellow-500/70 font-semibold"
                  >
                ...
                <div className="flex justify-end gap-3 pt-6 border-t border-yellow-500/20">
                  <button
                    ...
                    className="px-6 py-2.5 text-sm font-bold text-yellow-400 bg-[#08152F]/80 rounded-xl hover:bg-[#08152F]/70 transition-all"
                  >
                    Cancel
                  </button>
                  <button
                    type="submit"
                    disabled={creating}
                    className="px-6 py-2.5 text-sm font-bold text-black bg-gradient-to-r from-yellow-400 to-yellow-600 rounded-xl hover:from-yellow-400 hover:to-yellow-500 disabled:opacity-50 transition-all shadow-lg shadow-yellow-500/30 hover:shadow-yellow-500/50 hover:scale-105 active:scale-95"
                  >
