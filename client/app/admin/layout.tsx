...
function AdminLayoutContent({ children }: { children: React.ReactNode }) {
      ...
      icon: (
        ...
        >
          <path
            ...
            strokeLinejoin="round"
            strokeWidth={2}
            d="M15 5v2m0 4v2m0 4v2M12 3a9 9 0 110 18 9 9 0 010-18zM12 3v18"
          />
        </svg>
      ),
    },
    ...
    const fetchCounts = async () => {
      ...
      try {
            ...
            batch.map(async (type) => {
              try {
                ...
                if (isRateLimitError(err)) {
                  console.warn(
                    `Rate limit reached for ${type} transactions, using default value`,
                  );
                }
        ...
        try {
          ...
          if (isRateLimitError(err)) {
            console.warn(
              "Rate limit reached for vouchers, using default value",
            );
          }
          vouchersCount = 0;
        }
          ...
          await Promise.all(
            batch.map(async ({ apiKey, displayKey }) => {
              try {
                ...
                withdrawalCounts[displayKey] =
                  response.data?.pagination?.total || 0;
              } catch (err: any) {
                if (isRateLimitError(err)) {
                  console.warn(
                    `Rate limit reached for ${displayKey} withdrawals, using default value`,
                  );
                }
        ...
        try {
          ...
          if (isRateLimitError(err)) {
            console.warn(
              "Rate limit reached for user statistics, using default value",
            );
          }
          usersCount = 0;
        }
        ...
        if (
          ...
          console.warn(
            "Rate limit reached while fetching counts. Will retry on next interval.",
          );
        } else {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-50 via-blue-50 to-indigo-50 flex">
              ...
              <div key={tab.name} className="mb-1">
                {hasSubTabs ? (
                  <>
                    ...
                    >
                      <span
                        className={`flex-shrink-1 w-5 h-5 flex items-start justify-start ${tabActive ? "text-indigo-600" : "text-black"}`}
                      >
                        {tab.icon}
                      </span>
                      {sidebarOpen && (
                  ...
                  >
                    <span
                      className={`flex-shrink-0 w-5 h-5 flex items-center justify-center ${tabActive ? "text-indigo-600" : "text-black"}`}
                    >
                      {tab.icon}
                    </span>
                    {sidebarOpen && (
        ...
        <div className="border-t-2 border-indigo-200 px-3 py-4 bg-gradient-to-br from-indigo-50 to-purple-50">
          {sidebarOpen && user && (
            <div className="mb-3 py-2 bg-white rounded-lg border border-gray-200">
              ...
              <p className="text-sm font-medium text-black truncate px-3">
                Plateform Admin
              </p>
      ...
      {/* Main Content */}
      <div
        className={`flex-1 min-w-0 max-w-full overflow-x-hidden transition-all duration-300 ${sidebarOpen ? "md:ml-72" : "md:ml-20"}`}
      >
        {/* Mobile menu button */}
