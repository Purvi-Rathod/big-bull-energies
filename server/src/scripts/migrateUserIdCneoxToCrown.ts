...
async function migrateUserIds() {
  try {
    // Connect to MongoDB
    const mongoUri = process.env.MONGODB_URL_DEVELOPMENT || process.env.MONGODB_URI || process.env.MONGO_URI || process.env.MONGODB_URL_PRODUCTION;
    if (!mongoUri) {
      throw new Error("MONGODB_URL_DEVELOPMENT, MONGODB_URI, MONGO_URI, or MONGODB_URL_PRODUCTION environment variable is required");
    }
    ...
    // Find all users with CNEOX- prefix
    console.log("📋 Finding all users with CNEOX- prefix...");
    const usersToMigrate = await User.find({ userId: { $regex: /^CNEOX-/ } });
    console.log(`✅ Found ${usersToMigrate.length} users to migrate\n`);

    if (usersToMigrate.length === 0) {
      console.log("ℹ️  No users found with CNEOX- prefix. Migration not needed.");
